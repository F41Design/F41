...
def chips_html(names):
    ...
    return f"<div class='pchips'>{out}</div>"
...
:root {
    --bg:#07070e; --sf:#0d0d16; --s2:#11111a;
    --br:rgba(255,255,255,0.06); --br2:rgba(255,255,255,0.10);
    --tx:#b8c4d8; --bright:#ecf1f8; --mu:#3e4e63; --su:#617080;
...
.block-container{padding:0 1.2rem 3rem!important;max-width:1800px!important;}
[data-testid="stSidebar"]{background:var(--sf)!important;border-right:1px solid var(--br)!important;}
[data-testid="stSidebarContent"]{padding:1.3rem 0.9rem;}
section[data-testid="stSidebar"] > div{padding-top:0!important;}
div[data-testid="stVerticalBlock"]{gap:0!important;}

/* MASTHEAD */
.mhw{display:flex;align-items:center;justify-content:space-between;
    padding:20px 0 14px;border-bottom:1px solid var(--br);margin-bottom:10px;}
.mht{font-family:var(--mono);font-size:0.62rem;font-weight:700;
    letter-spacing:0.24em;text-transform:uppercase;color:var(--su);}
.mht b{color:var(--bright);font-size:1.05rem;letter-spacing:0.09em;}
.mhs{font-family:var(--mono);font-size:0.58rem;color:var(--mu);margin-top:3px;}
.mhr{font-family:var(--mono);font-size:0.56rem;color:var(--mu);text-align:right;line-height:1.9;}
.mhr span{color:var(--su);}

/* SUMMARY */
.sstrip{display:flex;gap:5px;padding:7px 0 13px;flex-wrap:wrap;}
.sp{display:inline-flex;align-items:center;gap:6px;padding:5px 11px;
    border-radius:100px;border:1px solid;font-family:var(--mono);
    font-size:0.59rem;font-weight:700;letter-spacing:0.05em;white-space:nowrap;}
...
.sl{color:#fd2453;border-color:rgba(253,36,83,.38);background:rgba(253,36,83,.09);
    animation:pp 2s ease-in-out infinite;}
@keyframes pp{0%,100%{box-shadow:0 0 0 0 rgba(253,36,83,.28);}50%{box-shadow:0 0 0 5px rgba(253,36,83,0);}}
...
.dhn{font-weight:700;font-size:0.68rem;color:var(--su);}
.dhd{font-family:var(--mono);font-size:0.54rem;color:var(--mu);margin-top:2px;}
.dht .dhn{color:var(--blue);}
.dht .dhd{color:rgba(59,130,246,.45);}
...
/* AUTO MATCH CARD */
.acard{padding:6px 9px 7px;border-radius:7px;margin-bottom:4px;
    border-left:2.5px solid #22c55e;background:rgba(34,197,94,.065);}
.albl{font-family:var(--mono);font-size:0.48rem;letter-spacing:0.1em;
    text-transform:uppercase;margin-bottom:2px;opacity:0.45;
    display:flex;align-items:center;gap:5px;}
.atxt{font-weight:600;color:var(--bright);font-size:0.67rem;}

/* PLAYER CHIPS */
.pchips{display:flex;align-items:center;margin-top:5px;}
.pch{width:20px;height:20px;border-radius:50%;object-fit:cover;
    border:1.5px solid rgba(34,197,94,.3);margin-right:-4px;
    background:var(--s2);flex-shrink:0;transition:transform .12s;}
.pch:hover{transform:scale(1.18);z-index:2;}
.pfb{display:inline-flex!important;align-items:center;justify-content:center;
    font-family:var(--mono);font-size:0.42rem;font-weight:700;color:var(--su);
    border:1.5px solid var(--br2);}

/* BADGES */
.bdg{display:inline-flex;align-items:center;gap:3px;padding:1px 5px;
    border-radius:3px;font-family:var(--mono);font-size:0.47rem;
...
.bd{color:#22c55e;background:rgba(34,197,94,.09);border:1px solid rgba(34,197,94,.22);}
.bs{color:#f59e0b;background:rgba(245,158,11,.09);border:1px solid rgba(245,158,11,.22);}
.lsc{font-family:var(--mono);font-weight:700;color:var(--bright);font-size:0.68rem;}

/* MANUAL ENTRY CARD — flex row with handle on left */
.mcard{
...
.mhandle{
    ...
    cursor:grab; opacity:0.2; transition:opacity .15s;
    font-size:0.7rem; color:var(--su);
    background:rgba(255,255,255,.015);
}
.mcard:hover .mhandle{opacity:0.55;}

/* card body */
.mbody{flex:1; padding:6px 8px;}
.mcat{font-family:var(--mono);font-size:0.47rem;letter-spacing:0.1em;
    text-transform:uppercase;opacity:0.42;margin-bottom:2px;}
.mcat-difine{color:#fd2453;}
.mcat-bromfc{color:#3b82f6;}
.mcat-manual_match{color:#22c55e;}
.mtxt{font-weight:600;color:var(--bright);font-size:0.67rem;line-height:1.4;}
.msub{color:var(--mu);font-size:0.57rem;margin-top:2px;}

/* ── MOVE / DELETE BUTTONS inside card ── */
.mbtns{
    display:flex; flex-direction:column; align-items:center;
    justify-content:space-between; padding:4px 3px;
    gap:2px; flex-shrink:0;
}

/* Streamlit button resets inside .mbtns */
.mbtns div[data-testid="stButton"]>button{
...
.mbtns div[data-testid="stButton"]>button:hover{
...
/* SIDEBAR */
.sbt{font-family:var(--mono);font-size:0.54rem;letter-spacing:0.17em;text-transform:uppercase;color:var(--mu);}
.sbt b{color:var(--bright);font-size:0.76rem;}
.sbl{font-family:var(--mono);font-size:0.55rem;font-weight:700;
    letter-spacing:0.11em;text-transform:uppercase;color:var(--mu);
    margin-bottom:5px;margin-top:11px;}
.legr{display:flex;align-items:center;gap:7px;font-size:0.6rem;color:var(--mu);margin-bottom:4px;}
.legp{width:6px;height:6px;border-radius:2px;flex-shrink:0;}

div[data-testid="stRadio"] label{font-size:0.69rem!important;}
div[data-testid="stTextInput"] input,
div[data-testid="stTextArea"] textarea{
    background:var(--s2)!important;border-color:var(--br2)!important;
    color:var(--tx)!important;font-size:0.69rem!important;}
div[data-testid="stSelectbox"]>div>div{
    background:var(--s2)!important;border-color:var(--br2)!important;font-size:0.69rem!important;}
div[data-testid="stButton"]>button{
    font-family:var(--mono)!important;font-size:0.6rem!important;letter-spacing:0.03em;}
</style>
...
with st.sidebar:
    st.markdown("<div class='sbt'><b>F41 · TAKVİM</b></div>"
                "<div style='font-family:var(--mono);font-size:0.5rem;color:#1a2030;"
                "letter-spacing:0.1em;margin-bottom:9px;'>YÖNETİM PANELİ</div>",
                unsafe_allow_html=True)
    st.divider()
    ...
    """, unsafe_allow_html=True)
    st.markdown(f"<div style='margin-top:12px;font-family:var(--mono);font-size:0.49rem;"
                f"color:#0f1623;'>{nw.strftime('%H:%M:%S')} · 60sn</div>",
                unsafe_allow_html=True)
...
for i, day in enumerate(week_days):
    ...
    op        = "0.3" if is_past else ("1" if is_today else "0.8")
    tc        = "dht" if is_today else ""
    tdot      = "<span class='tpip'></span>" if is_today else ""
    ...
    with cols[i]:
        with st.container(border=True):
            ...
            st.markdown(f"""
            <div class='dh {tc}'>
                <div class='dhn'>{TR_FULL[i]}{tdot}</div>
                ...
                if m["status"]=="inprogress":
                    ...
                    try:
                        ...
                        if 0 < dm <= 180:
                            ...
                            bdg = f"<span class='bdg bs'>{'%ds %ddk'%(h,mn) if h>0 else '%ddk'%mn} kaldı</span>"
                    except: pass

                st.markdown(f"""
                <div class='acard'>
                    <div class='albl'>⚽ Ekip {bdg}</div>
            ...
            for j, entry in enumerate(day_entries):
                ...
                # Score/badge for manual matches
                extra_html = ""
                if ec == "manual_match":
                    s_ = entry.get("status","notstarted")
                    sc = entry.get("score","")
                    if s_=="inprogress":
                        extra_html = f"<span class='bdg bl'>● CANLI</span> <span class='lsc'>{sc}</span>"
                    elif s_=="finished":
                        extra_html = f"<span class='bdg bd'>MS</span> <b>{sc}</b>"
                    else:
                        extra_html = entry.get("time","")
                    sub_html = f"<div class='msub'>{entry.get('sub','')} {extra_html}</div>"
                else:
                    sub_html = ""
...
<div style='text-align:center;padding:26px 0 6px;
    ...
    <span style='font-family:var(--mono);font-size:0.5rem;color:#090912;
